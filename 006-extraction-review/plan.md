...
**Storage**: PostgreSQL — 3 new tables (`ReviewerAssignment`, `ExtractionRecordLock`, `OrganisationAuditEntry`), a renamed column on `OutputRoute` (resend_policy), a new column on `DocumentType` (review_config), a new `OrganisationAuditEntry` link table in `apps/audit` (org scoping for the audit API; django-auditlog's own table untouched), and two column drops: `OutputRoute.delivery_mode` (preceded by a behaviour-preserving data migration) and `DocumentTypeField.confidence_threshold` (never read at flag time; the review gate lives only in `review_config`)
...
- **I. Protect Sensitive Data**: PASS — every new query org-scoped: `ReviewerAssignment` carries `organisation`; queue predicate runs through existing org-scoped querysets; the audit endpoint scopes `LogEntry` by joining the `OrganisationAuditEntry` link table written alongside each explicit entry — link membership is the visibility boundary, so auditlog's automatic diffs and admin-panel edits are unlinked and cannot surface — and it additionally excludes non-org / `is_staff` actors (tested explicitly); null-actor system rows are kept because the flagged-for-review moment has no actor. Registering `ExtractedFieldValue` with auditlog stores values already visible in the product; no new PII class.
- **II. Respect the Architecture**: PASS — follows established patterns: `SchemaField` config (as `PayloadConfig`), `AbstractLock` subclass (as `ApplicationLock`), dynamic Role + permission-string access policies, viewset `@action`s. One new pattern: a platform-wide audit read API over auditlog (`apps/audit`) — no prior art in the repo; approach agreed here in the plan phase per constitution II (thin read-only viewset over an unmodified auditlog `LogEntry`, org-scoped through an owned link table and filtered by subsystem = owning Django app; review-lifecycle actions logged explicitly through one writer, `log_action`, typed by an `AuditAction` base each feature subclasses, not derived from transitions).
...
├── models.py / enums.py / services.py / audit_view_set.py / serializers.py / filters.py / urls.py / migrations/
                                     #   OrganisationAuditEntry link table (org scoping + ordering),
                                     #   log_action single writer, AuditAction base enum,
                                     #   app-label subsystem filter, staff/non-org actor exclusion
...
7. **Audit (US7, FR-019..021)** — `apps/audit` read API over LogEntry: `OrganisationAuditEntry` link table for org scoping and index-driven ordering/date filters, app-label `subsystem` filter, staff/non-org actor exclusion; explicit `log_action` rows at the review-lifecycle moments (flag/confirm/correct/reject/select-for-review/resend); org-area Audit page with subsystem filter; tests incl. org isolation and actor exclusion.
