...
├── models.py / services.py / registry.py / audit_view_set.py / serializers.py / filters.py / urls.py / migrations/
                                     #   OrganisationAuditEntry link table (org scoping + ordering),
                                     #   log_action single writer, subsystem registry, staff/non-org
                                     #   actor exclusion
...
7. **Audit (US7, FR-019..021)** — `apps/audit` read API over LogEntry: `OrganisationAuditEntry` link table for org scoping and index-driven ordering/date filters, subsystem registry for the model filter, staff/non-org actor exclusion; explicit `log_action` rows at the review-lifecycle moments (flag/confirm/correct/reject/select-for-review/resend); org-area Audit page with model filter; tests incl. org isolation and actor exclusion.
