...
- [X] T039 [P] [US5] Test corrections produce before→after diffs on the explicit `log_action` entry (the diff is passed by the caller at creation, not harvested from auditlog's automatic `ExtractedFieldValue` diff), in `backend/django/tests/test_apps/test_document_entries/test_extraction_record_viewset.py`
...
- [X] T052 [P] [US7] Test that each review-lifecycle action emits an **explicit `LogEntry`** (via `apps.audit.services.log_action`) with a formatted `changes_text`, its own before→after diff, and exactly one `OrganisationAuditEntry` link row whose `timestamp` copies the entry's: flagged-for-review (at extraction write-back), confirm, correct, reject, select-for-review, resend — plus that an entry with no derivable organisation gets no link row and stays out of the feed, and that an auditlog auto-logged write never appears, in new `backend/django/tests/test_apps/test_audit/test_audit_view_set.py`
- [X] T053 [P] [US7] Test org isolation and actor exclusion: a user cannot see `LogEntry` rows for another organisation's records through `GET /api/v1/audit/` (scoped by joining `OrganisationAuditEntry`), and rows with non-org or `is_staff` actors are excluded while null-actor system rows are retained; plus that a non-admin org role is denied (`403`, `missing_permission: "admin"`), in `backend/django/tests/test_apps/test_audit/test_audit_view_set.py`

### Implementation for User Story 7

- [X] T054 [US7] Add the `OrganisationAuditEntry` link table in `backend/django/apps/audit/models.py` (one-to-one to `auditlog.LogEntry`, organisation FK, immutable timestamp copy, `(organisation, -timestamp)` index) with its migration — django-auditlog and `auditlog_logentry` stay untouched, replacing both the object-subquery and the swapped-`LogEntry`-model approaches — **user-confirmed migrate**
- [X] T055 [US7] Emit explicit `LogEntry` rows plus their link rows through the single writer `apps.audit.services.log_action` (one transaction) at the domain moments — flagged-for-review (extraction write-back, T023), confirm/reject (`update_status`), correct (`correct/` action), select-for-review (`update_status` into `needs_review`), resend (`route-deliveries/{id}/send/`) — each with a human-readable `changes_text`, in `backend/django/apps/document_entries/services.py` and the relevant viewsets (actions are logged explicitly, **not** derived from status transitions)
...
- [X] T057 [US7] Add a read-only `AuditViewSet` (`GET /api/v1/audit/`) in `backend/django/apps/audit/views.py`: filters by the requesting user's organisation (via the T054 column) + a `subsystem` query param matching the owning Django app label (`document_entries`, the only one surfaced now), plus `date_from`, `date_to`, `actor` params and pagination; excludes non-org and `is_staff` rows, keeps null-actor system rows; gated **admin-only** via `AuditAccessPolicy` (`user_has_permission:admin`); registered in `backend/django/apps/audit/urls.py`
- [X] T058 [US7] Add `client/src/app/audit/AuditPage.tsx`: subsystem filter (only Document entries for now), actor/date filters, a table rendering actor/time/action/before→after; system/staff actors never appear; `handle.permissions = [ADMIN]` mirrors the API gate
- [X] T059 [US7] Add an **Audit** entry under the Organisation area in `client/src/app/layout/DefaultNav.tsx` (`permissions={[ADMIN]}`), routed to `AuditPage`
