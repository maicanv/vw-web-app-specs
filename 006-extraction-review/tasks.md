...
- [X] T053 [P] [US7] Test org isolation and actor exclusion: a user cannot see `LogEntry` rows for another organisation's records through `GET /api/v1/audit/` (scoped by joining `OrganisationAuditEntry`), and rows with non-org or `is_staff` actors are excluded while null-actor system rows are retained, in `backend/django/tests/test_apps/test_audit/test_audit_view_set.py`
